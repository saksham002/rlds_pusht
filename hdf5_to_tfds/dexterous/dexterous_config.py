"""slurm_rlds config: Dexterous bimanual HDF5 datasets -> RLDS (common, profile-based).

One file for all dexterous HDF5 datasets (shirt_hang, lid, ...); select with the
DEXTEROUS_PROFILE env var. Folders are auto-discovered by regex under data_root.

Reuses the dexterous_hang parsing logic verbatim; the data format is identical.
The annotations file (lid_no_annotations.json) has empty datasets, so
has_subtask_annotations is False for every episode and per-step subtask fields
are dummy/null.

By default, converts HDF5 episodes from 60fps to RLDS tfrecords at 30fps (skipping
every other frame). Set DEXTEROUS_SUBSAMPLE=0 to disable subsampling and emit every
...
annotations/heuristic_annotations_60hz.json when DEXTEROUS_SUBSAMPLE=0. The
...
_SUBSAMPLE = os.environ.get('DEXTEROUS_SUBSAMPLE', '1') == '1'
...
assert not _SUBSAMPLE, (
    'DEXTEROUS_SUBSAMPLE must be 0: relative_action handling needs to be '
    'updated if subsampling is on.'
)

DATASET_VERSION = '1.0.0'

_DATA_ROOT_DEFAULT = '/data/group_data/rl/dexterous_robot_data'

# Each profile points the SAME parsing logic at a different dataset. Folders
# are discovered by REGEX under data_root (so new rounds matching the pattern
# are picked up automatically); a profile may instead pin an explicit
# dataset_dirs list (shirt_hang uses a curated subset of real_hang_*).
# Select with DEXTEROUS_PROFILE; override data_root/pattern/annotations via env.
PROFILES = {
    'shirt_hang': {
        'dataset_name': 'real_shirt_hang',
        'repo_id': 'dexterous_hang',
        'task_prompt': 'Place the shirt on the hanger and hang it from the rod.',
        ...
        'annotations_path': os.path.join(
            os.path.dirname(__file__), '..', 'dexterous_hang', 'annotations',
            'subtask_annotations_all.json'),
    },
    'lid': {
        'dataset_name': 'real_lid',
        'repo_id': 'dexterous_lid',
        'task_prompt': 'Place the lid on the pot.',
        'data_root': _DATA_ROOT_DEFAULT,
        'dir_pattern': r'real_lid_.*_hdf5',   # all lid hdf5 folders (excludes *_topleftmp4)
        'annotations_path': None,             # no subtask annotations
    },
}

_PROFILE = os.environ.get('DEXTEROUS_PROFILE', 'shirt_hang')
assert _PROFILE in PROFILES, f'unknown DEXTEROUS_PROFILE {_PROFILE!r}; choices: {list(PROFILES)}'
_P = PROFILES[_PROFILE]

DATASET_NAME = _P['dataset_name']
_TASK_PROMPT = _P['task_prompt']
_REPO_ID = _P['repo_id']
_DATA_ROOT = os.environ.get('DEXTEROUS_DATA_ROOT', _P['data_root'])
_ANNOTATIONS_PATH = os.environ.get('DEXTEROUS_ANNOTATIONS_FILE', _P['annotations_path'])


def _discover_dataset_dirs():
    """Resolve dataset folder names for the active profile.

    Explicit `dataset_dirs` wins; otherwise the regex `dir_pattern` (fullmatch)
    is applied to the immediate subdirectories of data_root, keeping only
    folders that actually contain episode_*.hdf5. Sorted for determinism.
    DEXTEROUS_DIR_PATTERN overrides the profile pattern.
    """
    env_pat = os.environ.get('DEXTEROUS_DIR_PATTERN')
    if _P.get('dataset_dirs') and not env_pat:
        return list(_P['dataset_dirs'])
    pattern = env_pat or _P.get('dir_pattern')
    assert pattern, f'profile {_PROFILE!r} has neither dataset_dirs nor dir_pattern'
    rx = re.compile(pattern)
    dirs = []
    for name in sorted(os.listdir(_DATA_ROOT)):
        full = os.path.join(_DATA_ROOT, name)
        if not os.path.isdir(full) or not rx.fullmatch(name):
            continue
        if any(fn.startswith('episode_') and fn.endswith('.hdf5') for fn in os.listdir(full)):
            dirs.append(name)
    assert dirs, f'no folders under {_DATA_ROOT} match {pattern!r} (with episode_*.hdf5)'
    return dirs


_DATASET_DIRS = _discover_dataset_dirs()
...
_STATE_DIM = int(os.environ.get('DEXTEROUS_STATE_DIM', '16'))
assert _STATE_DIM in (14, 16), f'DEXTEROUS_STATE_DIM must be 14 or 16, got {_STATE_DIM}'
_MAX_STATE_DIM = _STATE_DIM

_SUBSET_DIRS_ENV = os.environ.get('DEXTEROUS_SUBSET_DIRS', '').strip()
_SUBSET_DIRS = set(_SUBSET_DIRS_ENV.split(',')) if _SUBSET_DIRS_ENV else None

_TRAIN_COUNT_ENV = os.environ.get('DEXTEROUS_TRAIN_COUNT', '').strip()
_VAL_COUNT_ENV = os.environ.get('DEXTEROUS_VAL_COUNT', '').strip()
...
def _load_annotations():
    if not _ANNOTATIONS_PATH or not os.path.exists(_ANNOTATIONS_PATH):
        # No annotations file -> every episode unannotated (has_subtask_annotations=False).
        return {'subtask_definitions': [], 'datasets': {}}
